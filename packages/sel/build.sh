...
termux_step_pre_configure() {
	...
	# sh -o xtrace ./install.sh
	# popd
	curl -O https://beta.quicklisp.org/quicklisp.lisp
	./sbcl-1.4.3-x86-linux/run-sbcl.sh --load quicklisp.lisp --eval '(progn (quicklisp-quickstart:install) (ql-util:without-prompting (ql:add-to-init-file)))'
}
